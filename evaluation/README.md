...
- **`copyrighted-scans/`** (13 books) -- sourced from a real personal Zotero
  ...
  redistributable: no PDF, and its `public-cache/` text is a redacted
  snapshot rather than the real text (see below). Most have no DOI,
  `embedded_toc: false`, German and English
...
redact); every other book's entry is redacted instead (real
navigational/bibliographic text kept verbatim, chapter prose replaced with
random real words in the book's own language) since its PDF cannot be
redistributed. Generated by
`uv run python evaluation/scripts/generate_public_evaluation_cache.py` --
see `CLAUDE.md`'s "Document organization" section for details and known
failure modes.
...
breakdown), using only the existing 89-book corpus, no new ground
truth. See
