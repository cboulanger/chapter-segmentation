...
KISSKI for `evaluation/scripts/generate_dnb_toc_ground_truth.py` /
`evaluation/refresh_llm_cache.py`'s `--endpoint` flag (see
...
resolved by hand during dnb-toc-only arbitration
(`evaluation/experiments/dnb-toc-ground-truth.md`'s 2026-08-19 entry),
`Qwen3-Omni-30B-A3B-Instruct` was right in nearly every case;
`InternVL2_5-38B` had frequent word-level misreadings and at least two
outright hallucinated strings. Worth factoring into future
