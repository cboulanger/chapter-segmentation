...
**This is a snapshot of an in-progress ground-truth-generation experiment,
not a result of the main chapter-segmentation workflow.** `dnb-toc-only` is
a separate evaluation corpus of DNB-digitized table-of-contents scans (see
`evaluation/corpus/dnb-toc-only/README.md`), and this file tracks the
pipeline that builds ground truth for it -- `generate_dnb_toc_ground_truth.py`'s
two-independent-vision-model gate plus `arbitrate_dnb_toc.py`'s
human-in-the-loop resolution -- not the accuracy of `chapter_segmentation`
itself. See `evaluation/RESULTS.md` for that. "Current status" below is
expected to go stale and be rewritten as the pipeline changes or more of
the corpus gets ground truth; "History" holds the full write-up for every
superseded run and diagnosis along the way, kept in full so the reasoning
and dead ends behind the current numbers aren't lost.
